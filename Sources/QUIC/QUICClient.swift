...
/// A client QUIC session that owns the TLS 1.3 handshake orchestration and
/// drives a sans-I/O QUIC engine over injected datagram and time capabilities.
public final class QUICClient<
    Environment: QUICClientRuntimeEnvironment
>: Sendable {
    private let connection: QUICEngineConnection<Environment>
    private let tls: Mutex<QUICTLSClientSession>
    ...
    public init(
        ...
        tlsSession suppliedTLSSession: consuming QUICTLSClientSession,
        ...
        guard suppliedConfiguration.role == .client else {
            throw .engine(.invalidState("QUICClient requires a client engine configuration"))
        }
        ...
        configuration.localTransportParameters.originalDestinationConnectionID = nil
        configuration.localTransportParameters.retrySourceConnectionID = nil
        guard configuration.localTransportParameters.statelessResetToken == nil,
              configuration.localTransportParameters.preferredAddress == nil else {
            throw .transportParameters(
                .invalidValue(
                    parameter: "client_transport_parameters",
                    reason: "A client cannot send stateless_reset_token or preferred_address"
                )
            )
        }
    ...
    /// Starts TLS, sends the client Initial flight, then drives UDP, timers, and
    /// every subsequent TLS transition until the connection terminates.
    public func run() async throws(QUICConnectionDriverError) {
        ...
        guard canStart else {
            throw .engine(.invalidState("QUICClient.run() may be called only once"))
        }

        let startOutput: QUICTLSStepOutput
        do throws(QUICTLSHandshakeError) {
            startOutput = try withTLS {
                (session: inout QUICTLSClientSession) throws(QUICTLSHandshakeError)
                    -> QUICTLSStepOutput in
                try session.start()
            }
        } catch let error {
            throw .tls(error)
        }
        try connection.applyTLSOutput(startOutput)
    ...
    private func withTLS<Result: ~Copyable>(
        _ body: (inout QUICTLSClientSession) throws(QUICTLSHandshakeError) -> Result
...
extension QUICClient where Environment.Capabilities == NoExternalQUICTLSCapabilities {
    public convenience init(
        configuration: QUICConnectionEngineConfiguration,
        tlsSession: consuming QUICTLSClientSession,
        transport: Environment.Transport,
        timer: Environment.Timer,
        peer: IPSocketEndpoint
    ) throws(QUICConnectionDriverError) {
